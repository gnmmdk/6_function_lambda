...
class ExampleUnitTest {
    ...
    fun useLambda6() {
        ...
        var rtx =
            list.reduce { acc, e -> acc + e }//JavaKotlinAndroidGo    reduce函数：第一个参数acc是累加的返回值，e是当前遍历列表中的值
        println(rtx)

        var list2 = listOf(5, 3, 4)
        val reduce =
            list2.reduce { acc, e -> acc * e } //reduce函数：第一个参数acc是相乘的返回值，e是当前遍历列表中的值 注意上方的Stirng类型是无法用acc * e
        println(reduce)
    }
    ...
    fun useLambda7() {
        ...
        var list2 =
            test("Java", "Kotlin", "Android") { "疯狂${it}讲义" }//[疯狂Java讲义, 疯狂Kotlin讲义, 疯狂Android讲义]
        println(list1)
        println(list2)
    }
    ...
    /**
     * todo P125 6.10 高阶函数
     *  内联函数的使用：内联函数是以目标代码的增加为代价来节省时间开销的。
     *  内联函数的本质是将被调用的Lambda表达式或函数的代码复制、黏贴到原来的执行函数中，因此如果被调用的Lambda表达式
     *  或函数的代码量非常大，且该Lambda表达式或函数多次被调用--注意，每调用一次，该Lambda表达式或函数的代码
     *  就会被复制一次，因此势必带来程序代码量的急剧增加。
     *  何时使用内联函数：如果被调用的 Lambda表达式或函数包含大量的执行代码，那么久不应该使用内联函数；
     *  如果被调用的Lambda表达式或函数只包含非常简单的执行代码（尤其是单表达式），那么就应该使用内联函数。
     */
    @Test
    fun useInline() {
        var arr = arrayOf(20, 4, 40, 100, 30)
        var mappedResult1 = mapInline(arr, { it + 3 })
        println(mappedResult1.contentToString())
    }

    inline fun mapInline(data: Array<Int>, fn: (Int) -> Int): Array<Int> {
        var result = Array<Int>(data.size, { 0 })
        for (i in data.indices) {
            result[i] = fn(data[i])
        }
    ...
    @Test
    fun useInline3() {
        var arr = arrayOf(20,4,50,100,30)
        each(arr,{
            println(it)
            return//如果each函数没有inline修饰，此处编译错误
            //如果each函数有inline修饰，此处return将返回main函数
            //如果用crossinline修饰，编译失败 需要再理解下？
        })
    }
